-- Product-development analysis Agent v1.
-- The business page owns execution and human confirmation; Emperor Agent Run /
-- Checkpoint records are the durable orchestration state shared with the canvas.
...
VALUES
  (NULL,@product_analysis_agent_slug,'产品开发 · 七阶段分析','产品开发市场分析主链路，由业务页面托管执行与人工确认。','产品开发','active','project','manual',1800,@product_analysis_dag)
ON DUPLICATE KEY UPDATE
  ...
  `dagDefinition`=VALUES(`dagDefinition`),
  `updatedAt`=NOW();
...
SELECT
  ...
  CONCAT('v',@product_analysis_version_number),@product_analysis_dag_hash,'released',1,100,
