...
async function main() {
  ...
  const caller = lingxingSyncRouter.createCaller({ user, workspaceId: user.defaultWorkspaceId, requestId: "lingxing-ad-keyword-preview-verification", req: { headers: {}, header: () => undefined } as any, res: { locals: { requestId: "lingxing-ad-keyword-preview-verification" } } as any });
  const preview = await caller.createPreview({ dataDomain: "ad_keyword", scope: { storeId, profileId, startDate: "2026-08-01", endDate: "2026-08-21" } });
...
main().then(() => process.exit(0)).catch((error) => { console.error(error instanceof Error ? error.message : "LingXing ad keyword preview creation failed"); process.exitCode = 1; });
