...
try {
  ...
  if (target.requiredTable) {
    const [tables] = await connection.execute(
      "SELECT COUNT(*) AS count FROM information_schema.tables WHERE table_schema = DATABASE() AND table_name = ?",
      [target.requiredTable],
    );
    if (Number(tables[0]?.count || 0) !== 1) throw new Error(`Expected existing ${target.requiredTable} before reconciling ${migrationName}`);
  }
  ...
  console.log(JSON.stringify({ migrationName, status: "baselined", verifiedColumns: requirements.map(([table, column]) => `${table}.${column}`), verifiedTable: target.requiredTable || null }));
} finally {
