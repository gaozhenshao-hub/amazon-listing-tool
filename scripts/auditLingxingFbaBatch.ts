...
import { dataImports, opsAsinDailySnapshots, opsExternalSyncBatches, opsExternalSyncConfirmations } from "../drizzle/schema";
...
async function main() {
  ...
  const [snapshotCount] = await db.select({ count: count() }).from(opsAsinDailySnapshots).where(eq(opsAsinDailySnapshots.importId, importId));
  ...
  console.log(JSON.stringify({ batchStatus: batch.status, confirmations: confirmations.map((item) => ({ action: item.action, selectedCount: Array.isArray(item.selectedRowIds) ? item.selectedRowIds.length : 0 })), import: importRow || null, snapshotCount: Number(snapshotCount?.count || 0), ledger }));
}

main().then(() => process.exit(0)).catch((error) => { console.error(error instanceof Error ? error.message : "LingXing FBA batch audit failed"); process.exitCode = 1; });
