...
async function main() {
  ...
  const result = await invokeEmperorTool({
    ...
    params: { capability: "ad_campaign_keyword_report", arguments: { profile_ids: [profileId], report_date: "2026-08-01 - 2026-08-21", page: 1, length: 50, sort_field: "spends", sort_type: "desc" } },
...
main().then(() => process.exit(0)).catch((error) => { console.error(error instanceof Error ? error.message : "LingXing ad keyword probe failed"); process.exitCode = 1; });
