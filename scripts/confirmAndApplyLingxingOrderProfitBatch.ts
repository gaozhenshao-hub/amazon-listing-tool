...
async function main() {
  ...
  const caller = lingxingSyncRouter.createCaller({ user, workspaceId: user.defaultWorkspaceId, requestId: "lingxing-order-profit-batch-confirm-apply", req: { headers: {}, header: () => undefined } as any, res: { locals: { requestId: "lingxing-order-profit-batch-confirm-apply" } } as any });
  const preview = await caller.get({ batchId });
  if (preview.batch.dataDomain !== "order_profit" || preview.batch.status !== "ready_for_review") throw new Error("Batch is not a ready-for-review order profit product draft");
  const selectedRowIds = preview.rows.filter((row: any) => Boolean(row.selected) && row.rowStatus !== "needs_review").map((row: any) => row.id);
  if (selectedRowIds.length === 0) throw new Error("No selectable order profit product rows found");
...
main().then(() => process.exit(0)).catch((error) => { console.error(error instanceof Error ? error.message : "LingXing order profit batch confirmation/apply failed"); process.exitCode = 1; });
