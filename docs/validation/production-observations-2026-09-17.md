...
### Superseding Teamorouter endpoint repair — 2026-09-20

The official Teamorouter API integration contract and a controlled authenticated catalog read established that the current reachable OpenAI-compatible endpoint is `https://api.teamorouter.cn/v1`. Qingdao reaches this endpoint; the previous `.com` path reset connections and the application incorrectly forced it through the dead SOCKS tunnel. This was a stale endpoint/transport configuration, not a missing Teamorouter API key.

Two no-migration atomic releases moved the governed Teamorouter registry to the official endpoint and updated both the recoverable Skill Runner and the generic `emperor.run.run` path. The six catalog candidates were registered with environment-only credential references; no default model was changed. Each release passed Web/Worker/Scheduler active, local HTTP 200, and entrypoint hash verification.

With explicit approval, one synthetic N3 availability run then succeeded through the governed `analysis.rufus.attribute` Skill over Teamorouter. It used a synthetic-only input, model `gpt-5.5`, and persisted a successful external Skill Run. No user file was re-uploaded, parsed, read, retried, or used as input; no AI Job, acquisition job, Provider Run, keyword task, or Heartbeat was created. This supersedes the previous statement that N3 remained unavailable. A real user-file reanalysis is still a separate approval-controlled action.
