...
| --- | --- | --- | --- | --- | --- |
| `ad_auth_shops` | 广告账户 / 店铺 Profile | 否 | **P1 必需** | 解析 `Profile → SID × 站点`。 | Profile、SID、站点唯一且有效；通过即自动更新目录。 |
| `ad_campaign_product_report` | 广告商品 / ASIN / 广告组 / 广告活动 / 日 | **是，先关联子ASIN后回溯父ASIN** | **P1 产品详情主事实** | 子ASIN广告花费、销售、订单、流量及活动关系。 | 正常完整批次自动写入；异常才复核。 |
| `ad_campaign_report` | 广告活动 / 日 | 否 | **P1 辅助事实** | 活动预算、状态、策略、活动级花费及销售。 | 正常完整批次自动写入；异常才复核。 |
| `ad_campaign_group_report` | 广告组 / 日 | 间接 | 不在首期 | 后续活动下钻候选。 | 另行字段核验与范围确认。 |
| `ad_campaign_keyword_report` | 关键词 / 匹配方式 / 广告组或活动 / 日 | 间接 | 不在首期 | 独立关键词分析候选。 | 不进入产品详情首期；另行字段核验。 |
| `ad_campaign_search_term_report` | 搜索词 / 广告对象 / 日 | 间接 | 独立广告搜索词分析模块 | 搜索词归因与否词候选。 | 不进入产品详情；不属于本P1。 |
| `ad_campaign_targeting_report` | 自动投放或商品投放目标 / 日 | 间接 | 不在首期 | 独立投放诊断候选。 | 不进入产品详情首期；另行字段核验。 |
| `ad_portfolio_report_shop` | 组合 / 店铺 / 日期范围 | 否 | 不在首期 | 店铺或组合层预算与广告经营概览。 | 仅作店铺/组合视图，不能分摊到产品。 |
...
| 门禁 | 判定 | 自动应用动作 | 异常动作 |
| --- | --- | --- | --- |
| Profile 映射 | Profile 在目录中唯一映射为 SID × 站点。 | 自动写入目录并进入事实写入。 | 整批待复核。 |
| 分页完整性 | 已读实体行与源端总数、页数上限和返回窗口一致。 | 自动写入该报告日事实。 | 阻断部分写入，整批待复核。 |
| 总计行分离 | 总计行已识别且未混入实体行。 | 自动写入实体行与批次摘要。 | 阻断并记录源端形态变化。 |
| 实体身份 | Campaign、Profile、广告类型、广告ID、广告ASIN完整。 | 自动生成事实身份。 | 行隔离；若影响覆盖完整性则整批待复核。 |
| ASIN一致性 | `asin` 与创意中解析ASIN一致，或创意缺失但 `asin` 合法。 | 自动做父ASIN映射。 | 行待复核，不猜测归属。 |
| 父ASIN映射 | 按 SID × 站点 × 子ASIN 唯一命中。 | 自动纳入父ASIN周度KPI。 | 保留未映射行和证据，默认不计入KPI。 |
| 同源修订 | 同一事实键的新哈希与旧哈希不同。 | 自动追加修订审计并更新该来源事实当前版本。 | 仅异常阈值超限时待复核。 |
| 周度完整性 | 选定周内每个纳入范围的 Profile × 报告日均有成功读取、无分页截断；零实体行必须由源端成功响应证实。 | 自动形成“完整周”广告视图。 | 任一窗口失败或截断时，周视图保持待复核/部分数据，不作为正式周KPI。 |
| 指标异常 | 非负约束、非有限值、日环比异常和来源覆盖不足。 | 正常批次自动写入。 | 失败关闭，生成复核摘要。 |
...
| P1.2 | 实现按单日、单Profile、串行读取的“读取→自动校验→直接应用/异常复核”流程。 | QPS=1；正常完整批次无需人工确认即写入；全空总计行被过滤；分页截断阻断写入。 |
...
| P1.5 | 首先对 SP 的上一完整自然周执行7个日窗口的受治理读取；正常窗口自动应用，异常窗口进入复核。 | 真实批次覆盖、异常、Trace 和周度页面联动可验收。 |
| P2 | 若用户另行确认，再评估SD、广告组、关键词和投放报告的独立分析用途；搜索词继续建设于独立广告搜索词分析模块。 | 每类报告先执行 Schema + 最小实样核验；不默认进入产品详情。 |

## 9. 实施前需要您确认的选择

### 8.1 已确认的 P1 自动写入边界

P1 不会把“诊断”作为正常数据的人工前置步骤。一次受治理广告运行只读取 `ad_auth_shops`、`ad_campaign_product_report` 和 `ad_campaign_report`，并在同一 Profile、同一报告日内串行完成目录解析、分页读取、实体行过滤、父ASIN映射和门禁校验。所有门禁均通过时，系统直接追加或修订独立广告日事实，记录批次、行哈希、Trace 和应用结果；产品详情在周度查询时再聚合这些日事实。

| 运行结果 | 写入行为 | 产品详情周度视图 |
| --- | --- | --- |
| 正常完整批次 | 自动更新 Profile 目录，并直接写入/修订广告商品日事实与广告活动日事实。 | 当周7个报告日均完整时自动显示为正式周度数据。 |
| 存在未映射广告ASIN | 已精确映射的实体行照常写入；未映射行独立保留证据和复核状态。 | KPI不计入未映射行，并显示映射覆盖说明。 |
| Profile/SID冲突、分页截断、总计行形态变化、非有限指标或覆盖不完整 | 不写入该异常批次的事实；保留脱敏异常摘要、原始响应哈希和Trace。 | 该周显示“待复核/部分数据”，不展示为正式周度KPI。 |
| 同源迟到归因修订 | 对同一来源身份写入最新修订版本与差异审计，不累计旧新版。 | 周度KPI只读取该来源身份的当前版本。 |

> **P1 实现前提：** 需要新增独立广告事实、Profile映射、批次和修订审计的数据模型，并以迁移、定向Vitest、生产构建、受控发布和首次完整自然周运行完成验收；在用户明确确认“开始P1实施”前，不创建表、不新增定时任务、不发起广告业务读取或写入。

| 决策 | 选项 | 建议 |
| --- | --- | --- |
| 首期广告类型 | 仅 SP；SP + SD。 | **先仅 SP**。本次实样已验证 SP 的完整实体键；SD 先执行独立 P0 采样后接入。 |
| 应用策略 | A：完整批次自动应用、异常人工复核；B：每日人工一键应用。 | **已确认 A**：正常完整批次直接写入，异常才人工复核。 |
