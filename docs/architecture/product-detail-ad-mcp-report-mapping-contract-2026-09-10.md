...
单产品详情中的广告数据应以 **广告商品报告**（`ad_campaign_product_report`）作为产品维度的权威明细来源，以 **广告活动报告**（`ad_campaign_report`）补足活动状态、预算和活动级信息，并通过 **广告授权店铺目录**（`ad_auth_shops`）把广告 Profile 精确关联至系统已有的店铺 SID 与站点。
...
| `ad_campaign_search_term_report` | 搜索词 / 广告对象 / 日 | 间接 | P2 | 搜索词归因与否词候选。 | 需有活动或广告组实体键；只生成建议。 |
...
父ASIN详情的广告 KPI 只汇总状态为 `exact_asin_same_day`、`exact_asin_prior_evidence`、`exact_sku` 或 `manual_confirmed` 的广告商品事实；按所选日期范围、店铺和站点对其**子ASIN级事实**求和。
...
| 规则类型 | 规则 | 处理方式 |
| --- | --- | --- |
| 报告日期 | 广告活动/商品实体行不保证包含业务日期字段。 | 强制单日读取；`reportDate` 写请求窗口 D，不取 `updated_at` 或 `creation_date`。 |
...
默认只读取精确映射和人工确认映射，且必须返回数据来源、最新报告日、Profile 覆盖、映射状态分布、批次 ID 和 Trace 引用。未映射行默认不计入 KPI，但可在“映射异常”折叠区域查看。

### 7.2 返回视图

| 视图 | 来源 | 包含范围 | 不包含范围 |
| --- | --- | --- | --- |
| 广告商品 KPI | `ops_ad_mcp_product_daily_facts` | 精确/人工确认映射的子ASIN实体行。 | 总计行、名称候选、未映射行、活动级重复金额。 |
| 活动列表 | `ops_ad_mcp_campaign_daily_facts` + 商品映射统计 | 相关活动、状态、预算、关联子ASIN数。 | 用活动全额直接算父ASIN KPI。 |
| 经营广告汇总 | `ops_asin_daily_snapshots` | 日表现中已确认的广告原子指标。 | MCP 广告商品明细金额的再累计。 |
...
| P1.4 | 在统一产品详情新增广告商品 KPI、活动清单、数据状态条与映射异常入口。 | 与 ASIN 日表现广告汇总并列、不双算。 |
| P1.5 | 首先对 SP 实施 7 天只读预览，再由用户选择自动应用或人工一键应用。 | 真实批次覆盖、异常、Trace 和页面联动可验收。 |
| P2 | 增加 SD、广告组、关键词、搜索词和投放报告。 | 每类报告先执行 Schema + 最小实样核验，再扩展字段映射。 |
...
| 首批历史范围 | 不回填；近 7 天；近 30 天。 | 不回填；先完成 7 天只读预览。 |
