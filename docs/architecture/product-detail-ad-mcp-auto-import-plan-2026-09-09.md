...
本方案的目标是让运营人员在一个父 ASIN 的统一详情中，按“店铺 × 站点 × **完整自然周** × 子 ASIN”准确看到广告指标、关联活动和映射质量；广告源事实仍按报告日保存，但详情消费窗口必须与产品总览一致为周一至周日。首期**只接入授权店铺、广告商品和广告活动报告**；搜索词、关键词和投放目标均不进入首期产品详情。正常完整批次自动直接入库，异常数据才进入可编辑、可确认的复核队列。
...
| MCP 广告关键词/搜索词/投放事实 | 各自独立的Profile × 日期 × 广告实体粒度 | 首期不读取、不写入产品详情；后续如需建设，仅进入各自独立广告分析模块。 | 不自动执行否词、预算、竞价、匹配方式或广告状态变更。 |
...
| P4：独立广告分析扩展（另行确认） | 如有需要，再单独建设关键词、投放、广告组或搜索词分析模块。 | 不属于产品详情首期，也不影响产品总览周度KPI。 | 每类报告先实样核验；不自动执行广告写操作。 |
