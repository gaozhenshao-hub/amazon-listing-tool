# f产品知识库蒸馏为 Listing 与图片协同 Skill 方案
...
1. 当前产品的已验证事实、兼容性和用户锁定内容；

1. 当前用户在本次任务中显式编辑/确认的 `claimLedger` 与大纲；

1. 已发布、适用范围匹配的 Skill 版本；
...
1. 首批发布权限是否坚持“超级管理员发布、其他角色可提交草案”；

1. P1 是否先限定为“生成五点/A+内容规划”，暂不直接进入现有逐条精雕生成。
