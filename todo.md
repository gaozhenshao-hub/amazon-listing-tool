...
- [ ] 验证空气套件的A+ 1至7及品牌故事可完整保存、刷新水合并展示

## Step5历史失败状态重跑修复（2026-08-16）

- [ ] 审计空气套件本次前台“重新生成”未创建新AI Job的触发链路
- [ ] 修复历史失败状态或旧运行ID阻塞Step5创建新任务的问题
- [ ] 验证新AI Job创建后完整A+结果保存、刷新水合和前台展示

## Step5旧失败状态残留修复（2026-08-16）

- [ ] 核验已部署服务是否使用LONGTEXT结果Schema，并确认截图中的94564字节错误仅为历史任务记录
- [ ] 修复Step5失败后前台残留旧运行ID、旧错误和旧结果，确保重新生成切换到新任务
- [ ] 补充失败态存在旧运行ID时重新生成仅追踪新运行的前台回归测试
- [ ] 用空气套件验证新任务完成后仅展示当前运行状态及完整A+内容
