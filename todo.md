...
## 模块三：产品总览品名优先展示（2026-08-14）
- [ ] 将父 ASIN 产品总览卡片主名称改为优先使用上传表品名
- [ ] 保留 Listing 标题作为品名缺失时的回退字段，并补充回归验证
