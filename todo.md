...
- [x] 确认子ASIN日数据、库存快照、广告事实、父ASIN周报与产品主档的身份键、来源优先级和去重边界，形成逐项根因结论。（确认手工成员表失真与裸ASIN跨店混合；确认MCP父周事实同键唯一。）
 ...
 - [ ] 对截图复现路径、单ASIN详情、父ASIN周度总览、库存规划和广告归因执行回归与青岛生产只读验证，保留审计和回滚能力。
...
- [ ] 为统一产品视图补充来源优先级、跨店铺/站点身份隔离、详情指标合成、筛选及产品管理权限的定向回归，并在青岛已认证页面验证单入口展示。（本地12项定向回归、ESLint与生产构建已通过；待青岛受控部署后进行已认证页面验证。）
...
- [ ] 修复同一美国站被“US”与“美国”等别名拆分导致的子ASIN重复和成员数错误：统一来源身份键必须规范化站点代码，合并同一事实但不重复累计指标，并在总览与详情回归验证。（共享规范化、详情范围筛选、总览合并与33项本地回归已完成；青岛认证详情截图确认子体已由8行收敛至4行且销售、利润、广告、库存字段完整；总览卡片页面验收另列待办。）
