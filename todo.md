...
- [ ] 在子 ASIN 状态中展示停售原因并提供人工恢复为在售的确认入口
- [ ] 验证自动停售与人工恢复路径的回归结果并发布
...
- [ ] 核对产品表现表和现有日快照中采购成本、单件成本与币种字段覆盖
- [ ] 定义本月及未来两个月采购需求、采购月份和资金占用的计算口径
- [ ] 实现采购成本来源与子ASIN人工成本维护的可追溯模型
- [ ] 在库存规划工作台新增月度采购表、月度汇总和资金规划展示
- [ ] 验证月度采购数量、采购成本、资金汇总和人工成本调整的回归结果

## 模块三：产品总览基本信息与成本参数（2026-08-13）
- [ ] 建立子ASIN产品成本、预估/实际头程、预估/实际FBA、售价及币种的数据模型
- [ ] 实现预估平手价和实际平手价公式，并保存可审计的人工调整
- [ ] 在产品总览卡片左侧增加可展开产品基本信息，展示并编辑成本参数
- [ ] 让产品总览库存指标与库存规划工作台使用同一日快照数据及同一最近日期基准
- [ ] 让月度采购表直接读取已确认产品成本计算采购资金占用
