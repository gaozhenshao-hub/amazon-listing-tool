...
- [ ] 核验主图、辅图2至7、A+ 1至7及独立品牌故事的参考图覆盖、连续编号和内容非空
- [ ] 核验Step4参考图与Step5图片建议的模块映射一致性（数量与展示已确认，待逐项比对模块与子图标识）
- [ ] 逐项核验空气套件Step4 imageReferences与Step5 A+模块、品牌故事的moduleNumber、parentModuleNumber和subModuleNumber映射
- [x] 补充Step4重建参考图与Step5最终建议在多图子模块及品牌故事上的映射一致性测试（联合断言覆盖子图与品牌故事稳定imageKey，并修复同编号辅图误匹配）
- [ ] 对空气套件当前会话执行Step4展示目标与Step5最终结果的逐项映射检查，记录A+ 1至7、合法子图和品牌故事对应关系
...
- [ ] 用空气套件重新生成Step4并确认车库、庭院、露营、工地四个参考图非空（任务成功且场景键存在；待逐卡内容与前台展示验证）
