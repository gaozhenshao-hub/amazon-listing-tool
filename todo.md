...
### 模块三：产品总览增加库存列
- [x] 审查现有产品总览表格和产品表现数据结构
- [x] 后端：从已上传的产品表现表格提取库存字段（可售库存、在途库存）
- [x] 后端：计算可售天数（可售库存 ÷ 7天平均日销）
- [x] 后端：增加生产时间字段存储（可编辑）
- [x] 后端：AI判断库存状态（结合可售天数、生产时间、在途库存）
- [x] 前端：产品总览表格增加库存列（可售库存、在途库存、可售天数）
- [x] 前端：增加可编辑的生产时间列
- [x] 前端：库存状态标签展示（颜色区分）
- [x] 前端：AI补货建议展示（通过状态标签+生产物流配置实现）
- [x] 单元测试（15个测试全部通过）

### Bug修复：登录“JSON输入意外结束”
- [x] 排查登录接口和认证流程（确认是Cloud Run 503冷启动问题）
- [x] 修复登录问题（添加自动重试+友好错误提示+非JSON响应处理）
- [x] 修复 productOps.ts TS类型错误

### 修复剩余59个TS类型错误
- [x] 收集完整TS错误列表（59→41个，通过target ES2020解决18个）
- [x] 修复 productOps.ts 类型错误（部分）
- [ ] 修复其他文件类型错误（剩余41个，已并行分发修复）
- [ ] 验证0错误编译通过
...
## 前端筛选器同步与卖点筛选
- [x] 前端筛选器与标签体系同步（颜色筛选使用13种颜色标签，类目使用18种类目）
- [x] 前端筛选器增加"卖点表达"维度筛选（卖点大类+子类联动）

## 单图标签面板布局优化 + 提亮色筛选
- [x] 单图标签编辑面板布局优化：改为更紧凑的横向标签式布局，减少垂直空间占用
- [x] 筛选器增加“提亮色”维度筛选
...
- [x] 集成：图片标签编辑面板从数据库读取标签选项（替代硬编码常量）
- [x] 前端：图片筛选下拉框每个选项增加统计数量后缀（如"大厂极简风 (5)"）
- [x] 前端：单图标签编辑面板移除"主颜色"和"设计风格"（它们属于套图级别标签，套图打标后自动应用到所有子图）
- [x] 前端：ASIN图片集详情页改为亚马逊风格展示（主副图左右切换大图预览，品牌故事+A+图片垂直排列）
- [x] Bug修复：套图级别标签（主颜色/设计风格/提亮色/类目）筛选时应继承到套图下所有单图（后端查询需JOIN kb_image_sets获取套图标签）
- [x] 前端：标签管理页面-图片类型/卖点分类的二级子标签增加编辑功能（重命名/删除/新增子标签）
- [x] 前端：标签管理页面-A+图片类型增加二级分类标签（图片轮播/对比表格/全宽图/图文叠加/四图文/三图文/热点交互/视频模块/导航轮播等）

## 图片归属维度改造：A+模块从图片类型迁移到图片归属 (2026-07-02)
- [x] 后端：从IMAGE_TYPE_HIERARCHY中移除“A+模块”及其12个子标签
- [x] 后端：新增IMAGE_BELONG_HIERARCHY常量（主图/套图/A+→12子类/品牌故事）
- [x] 后端：kbTags初始化逻辑支持imageBelong为层级结构
- [x] 数据库：新增 tagImageBelongSub 字段
- [x] AI分析提示词更新：归属为A+时识别子模块类型
- [x] 前端：图片归属筛选器改为二级联动（A+展开显示12个子模块）
- [x] 前端：标签管理页面图片归属维度显示为二级结构
- [x] 修复相关测试（IMAGE_TYPE_MAIN_OPTIONS恢复为6个）
- [x] 验证筛选和标签编辑功能正常

## Bug修复与功能优化 (2026-07-02)
- [x] Bug: ASIN图片集加载过慢（点击卡片后弹窗loading时间长）
- [x] 功能: A+和主副图排序功能（支持拖拽排序调整图片顺序）

## 图片知识库功能优化 (2026-07-08)
- [x] 功能: 图片归属选择后自动归位（选A+移到A+区域，选品牌故事移到品牌故事区域）
- [x] 功能: A+区域内图片位置调换（上下箭头按钮快速调换相邻图片顺序）
