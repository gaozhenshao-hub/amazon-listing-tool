...
- [ ] 等待生产前端资源完成切换后，复核空气套件Step5实际显示A+ 7模块级正文且品牌故事仍独立展示
...
- [ ] 审计Step4失败任务残留的运行ID、生成态与重新推荐Mutation调用
- [ ] 修复历史失败运行不再让Step4前台持续显示生成中或阻塞新任务创建
- [ ] 用空气套件自主点击重新推荐，确认创建新的Step4 AI Job
- [ ] 验证新Step4 Job成功保存四种场景子图的可编辑参考方案
...
- [ ] 审计Step4参考图结果已写入step4AiResult但AI Job仍处于running的Agent同步路径
...
- [ ] 用空气套件自主重跑并确认Step4任务成功完成且页面不再持续生成中（任务成功，待前台终态证据）
- [ ] 逐项查询并记录Step4任务360007中A+5车库、庭院、露营、工地参考卡片的构图、效果和可编辑内容字段非空
- [ ] 用空气套件前台刷新Step4并确认生成态结束、四种场景参考图卡片正常展示
- [ ] 为Step4场景参考图成功结果补充四个子图内容非空与前台终态水合的回归断言
