...
- [ ] 审计青岛生产库缺失的领星MCP父ASIN自然周事实，按授权店铺、规范站点、父ASIN和自然周输出脱敏缺口范围，并核对历史来源与身份唯一性。
- [ ] 对确认的父ASIN周度缺口执行领星MCP只读预览、完整性/异常校验和一次性应用确认；仅幂等新增权威周事实，不覆盖历史上传周表或已有MCP事实。
- [ ] 补齐后在统一产品总览核验周度覆盖、来源标签和重复保护，归档Run/Trace及回滚依据。
- [ ] 修复父ASIN周报受治理应用在批内将“US”与“美国”视为不同站点、但写入唯一键归并后触发重复身份阻断的问题；预检和应用必须共用规范站点键，且同键候选仅保留来源优先的单条事实，绝不重复累计。（第二次诊断已将别名证据读取从已规范化的`normalizedData`修正为原始`sourceData`；5项定向回归、ESLint和生产构建通过；待青岛受控发布后重新预检与应用。）
