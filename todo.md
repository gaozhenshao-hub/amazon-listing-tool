...
- [ ] 将Step5最终图片建议改为主图、辅图和A+模块的分段生成及服务端合并
...
- [ ] 将参考图与图片建议中的A+模块统一重新编号为A+ 1至A+ 7（归一化与回归测试已完成，展示层和真实会话待验证）
- [ ] 将品牌故事从A+模块序列中剥离为独立内容，避免编号延续或重复（Step5数据层已完成，Step4展示层和真实会话待验证）
- [ ] 补齐A+ 7的模块级Purpose、Content、构图与效果建议，不只显示子图占位（服务端回填已完成，前台渲染和真实会话待验证）
- [ ] 使用截图场景验证参考图序号、图片建议编号和品牌故事展示均正确
- [ ] 修复Step4参考图展示层：按当前大纲稳定显示A+ 1–7，并将品牌故事作为独立卡片展示
- [ ] 修复Step5前台渲染与历史结果水合：确保A+ 7展示模块级内容而非仅子图占位

## Step5历史区块去重与唯一重建（2026-08-16）

- [ ] 清除Step5历史结果中8至13等非当前大纲编号的A+区块
- [ ] 按当前Step2大纲顺序唯一构建A+ 1至7，避免历史区块与回填区块叠加
- [ ] 将历史品牌故事区块转为独立品牌故事，不占用A+编号
- [ ] 用截图场景验证不会出现A+ 13后又出现A+ 1.1的重复序列

## Step4历史结果重建与校验恢复（2026-08-16）

- [ ] 在Step4水合时按当前Step2大纲重建A+ 1至7与独立品牌故事展示顺序
- [ ] 在Step4历史AI输出校验失败时保留并恢复可编辑的历史参考图结果
- [ ] 用空气套件验证参考图不再显示A+ 13或A+ 1.1等旧编号

## Step4会话快照强制重建（2026-08-16）

- [ ] 在getSession返回前以Step2当前大纲作为唯一编号来源重建Step4参考图区块
- [ ] 阻断旧step4UserEdit、历史版本和Artifact水合结果直接覆盖重建后的展示快照
- [ ] 验证刷新后参考图仅显示A+ 1至7及独立品牌故事
