...
## 本地、Manus与青岛ECS同步审计（2026-08-22）

- [x] 审计本地检查点与青岛ECS已部署源码的差异，形成“已部署/仅本地/待恢复”清单（审计结论已记录于`docs/validation/qingdao-recovery-audit-2026-08-25.md`）
- [x] 将确认已部署但未保存的优化从青岛ECS受控回填到本地工作区，保留当前设计检查点且不触碰环境文件、密钥或业务数据（已恢复应用源码；工作区审计未发现环境文件、密钥或构建产物）

- [ ] 调研Codex公开任务编排与Manus上下文工程设计，审计现有皇帝Skill、Agent、Tool Gateway、Run Ledger、权限和附件能力的可复用边界
- [ ] 输出通用对话任务管理器的架构流程、页面交互、权限、附件上下文、Skill/Agent/MCP调度与人工确认规范，并获得框架确认
- [x] 新增会话、消息、附件、调度计划和执行关联的数据结构；附件仅保存受控对象存储引用，所有运行沿用Run Ledger审计（0158–0168已受控登记；恢复、快照与上下文来源溯源Schema已核验）
- [x] 实现皇帝中台对话任务管理器：多轮对话、文件/图片上传、可选择Skill/Agent/MCP、执行预览、运行时间线和人工确认（青岛ECS已部署源码已回填；定向前后端契约回归通过）
- [x] 将对话任务管理器纳入现有Tool Gateway、权限目录、上下文编译、审批协议和受控并行计划，不开放任意Shell或未登记写入工具（能力白名单、审批阈值、附件MIME策略和Run Ledger已验证）
- [ ] 为对话任务、附件、工具白名单、审批和Trace写入补充前后端回归测试，部署青岛ECS并完成端到端验收
- [ ] 补充对话任务管理器运行时间线的前台/路由验收证据，并在规划Skill网关不可用时展示受控、可恢复的降级说明
- [ ] 核验并补齐对话任务流程与统一权限目录、受控并行草稿的实际接线；若未接线，保持功能不对用户承诺
...
- [x] 修复恢复审计发现的Harness运行时Preset未传递至Skill Runner的问题，确保quality_first/evaluation的模型策略通过Agent执行链路真实生效并可审计（Preset仅排序已登记模型候选；Agent Job、Skill Run输出和观测元数据均记录该模式）
- [x] 核对并补齐Harness受治理Tool目录与契约：仅在存在明确、非写入型导出准备能力时恢复`internal.export.prepare`，否则将测试调整为当前实际能力边界（未发现对应执行实现，保持不注册；契约改为断言MCP店铺范围强制）
- [ ] 在青岛ECS或资源充足环境执行完整非监听TypeScript编译与生产构建；当前沙箱`pnpm check`被SIGTERM中止且未产生诊断
- [ ] 待青岛ECS远程会话恢复后，仅以只读方式复核0163–0168生产账本、关键Schema和服务健康；禁止重复迁移、重启或覆盖生产数据
