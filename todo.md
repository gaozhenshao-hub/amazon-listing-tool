...
- [ ] 为Step5历史会话补充组合视图渲染回归测试，断言A+ 1至7连续可见且品牌故事独立展示
...
- [ ] 核验多图A+子模块编号、场景主题和备注驱动数量与当前大纲一致（模块结构与子图数量已核验；场景备注和主题待逐项验证）
- [ ] 用空气套件实际填写场景备注，逐项核验moduleNumber、subModuleRemark、subModuleCount、各子图标题与前台显示一致
...
- [ ] 验证场景子图目标同步到Step4参考图与Step5图片建议
- [x] 核验逐图锁定后刷新、解锁、重新优化和整体确认不会丢失子模块内容（用户确认全流程正常）
- [ ] 核验Step2当前子模块目标与Step4参考图及Step5建议的映射一致
...
- [ ] 用空气套件当前Step2会话逐项记录A+子模块编号、标题/主题、subModuleRemark和subModuleCount，并与前台显示核对
