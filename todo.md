...
- [ ] 按用户确认对青岛独立站执行本轮受控应用部署：先备份当前编译目录与服务状态，仅发布版本c4dcce75的应用代码，不运行迁移或数据写入；重启受管服务后完成脱敏健康与页面回归，异常立即回滚备份。
...
- [ ] 按用户确认部署统一产品视图及加载门控修复版本b658c10c至青岛独立站：保留既有回滚备份，仅替换应用编译产物并重启受管服务；禁止迁移、业务数据写入和领星同步，并在已认证页面完成单总览/单详情回归。
- [ ] 修复同一美国站被“US”与“美国”等别名拆分导致的子ASIN重复和成员数错误：统一来源身份键必须规范化站点代码，合并同一事实但不重复累计指标，并在总览与详情回归验证。（共享规范化、详情范围筛选、总览合并与33项本地回归已完成；待青岛发布后真实页面验收。）
