...
- [ ] 流式投影前台：在运行历史/对话时间线接入受控短轮询或SSE等价投影、断线重连和权限过滤可视化；不得直接消费未审计的运行内存状态
- [ ] 失效阻断：在Plan/Step恢复前检查关联Manifest的来源失效状态，出现失效时拒绝恢复、要求重新编译上下文并再次人工确认
- [x] 失效阻断实现：对话步骤恢复在关联Trace存在`invalidated`来源时写`recovery_rejected`并拒绝恢复，要求重新编译与再次人工确认；治理回归8/8通过
- [ ] 失效阻断验收：部署青岛ECS并使用归档会话标记来源失效，验证恢复请求被拒绝且不执行模型、Tool或MCP；补充前台可读提示
- [ ] 评测SLO：基于已有质量门禁、人工评分和Run Ledger构建可配置SLO指标、阈值告警与只读看板，不伪造评分或用户反馈
- [x] 评测SLO基础：新增管理员只读SLO查询，基于真实`emperor_ai_os_evaluations`与运行统计计算评测均分、Skill/Agent失败率及样本不足状态；不生成评分、不改变门禁或执行（青岛ECS构建与服务健康通过）
- [ ] 评测SLO前台：在质量门禁或观测页面展示SLO趋势、样本不足、阈值违约与人工评测来源；告警仅提示，不自动发布、回退或重试
- [ ] 受控子图并行：仅对已经审批、无写入冲突且有明确依赖图的分支生成并行草稿；默认串行，执行前保持人工确认，写入/高风险节点禁止自动并行
