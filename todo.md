...
- [ ] 修复美工角色已授予“智能图片建议”查看权限但进入页面发生insertBefore DOM崩溃的问题（代码修复、9项图片工作流回归、ECS完整构建和三服务重启已通过；待真实美工会话加载已有Step5结果验收）
- [ ] 回归美工角色的模块权限、二级页面访问与图片建议页面稳定渲染，确认不扩大编辑或删除权限
- [ ] 使用真实美工账号在青岛ECS打开已有Step5结果的图片建议页面，确认无insertBefore/removeChild异常
- [ ] 在真实美工会话核验只读图片建议结果的可见性与现有编辑、删除权限边界，并补充含辅图和A+历史数据的组件级渲染回归
- [ ] 创建仅用于本次验收的临时美工账号，复用designer角色的既有图片建议查看权限且不新增编辑授权
- [ ] 使用临时美工账号在Manus生产站加载空气套件Step5结果，记录页面稳定性与权限边界后停用该账号
- [ ] 收口图片建议页的操作级权限：美工仅具备read/delete时不得显示或执行重新生成、微调、上传成品、AI优化、编辑文本与确认锁定等编辑操作
- [ ] 复核临时美工跨登录后的项目选择与Step5数据来源，排除管理员会话缓存导致的非授权项目内容残留
