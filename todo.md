...
- [ ] 新增会话、消息、附件、调度计划和执行关联的数据结构；附件仅保存受控对象存储引用，所有运行沿用Run Ledger审计
- [ ] 实现皇帝中台对话任务管理器：多轮对话、文件/图片上传、可选择Skill/Agent/MCP、执行预览、运行时间线和人工确认
- [ ] 将对话任务管理器纳入现有Tool Gateway、权限目录、上下文编译、审批协议和受控并行计划，不开放任意Shell或未登记写入工具
