...
## 阶段间数据联动 (2026-03-17)
- [ ] 后端: 新增BOM成本汇总查询接口（材料总成本、模具总成本、单件成本）
- [ ] 后端: 新增产品画像成本数据提取接口（从产品成本子模块提取结构化数据）
- [ ] 前端: ProfitEditor自动读取BOM成本汇总，填充到成本输入字段
- [ ] 前端: ProfitEditor显示数据来源标识（BOM联动/手动输入）
- [ ] 前端: BomEditor读取产品画像中的成本相关数据作为参考
- [ ] 前端: 联动数据变更时提示用户刷新下游模块
- [ ] 测试: vitest验证数据联动接口和流转逻辑
