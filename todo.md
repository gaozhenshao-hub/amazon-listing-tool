...
- [ ] 补充并验证FBA库存映射到月度采购与资金规划的实际消费链路：明确读取`ops_asin_daily_snapshots`的字段、计算入口和回归测试后再标记完整联动
- [ ] 补充并验证广告活动/关键词报表到广告看板与产品总览的实际下游消费映射：提供查询/适配代码和回归证据，确认预算、竞价、否词和投放状态保持只读
