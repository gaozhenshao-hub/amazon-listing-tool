...
- [ ] 修复Step4上传本地构图或效果参考图并按图片与备注单图优化时的removeChild前端崩溃，确保优化完成后页面稳定保留全部参考图与备注
- [ ] 修复Step4本地参考图单图优化回写后会话水合覆盖为全图锁定、隐藏本地参考图与备注的问题，确保优化仅更新目标图方案并保留人工资产状态
- [ ] 修复Step4单图优化回写仅读取历史备注而不回填本次输入备注的问题，确保本地参考图备注参与皇帝Skill后仍持久化显示
- [ ] 修复Step4页面加载时历史成功任务输出覆盖已保存本地参考图草稿的问题，确保仅当前页面启动的任务可回写前端编辑状态
- [ ] 降低托管Vite生产构建峰值内存并成功切换最新Step4前端工件，解除生产域名持续引用旧动态分块的阻塞
