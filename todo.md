...
## 产品总览周度父ASIN来源优先级调整（待方案确认，2026-09-04）
- [ ] 审计产品总览周度聚合、父ASIN周汇总表、用户上传周表及ASIN日快照的现有来源优先级和指标字段差异。
- [ ] 输出周度总览固定使用父ASIN周汇总、单ASIN详情固定使用ASIN日数据、库存规划继续参考ASIN日数据的数据流与页面口径方案，待用户确认后实施。
- [ ] 确认后调整前端、后端与数据源优先级，禁止ASIN日数据继续派生或覆盖产品总览父ASIN周度指标。
- [ ] 补充周度来源隔离、单ASIN日数据详情和库存规划引用的回归与真实数据验证，保留历史上传数据、审计和回滚能力。

## 领星MCP父ASIN周报替代日聚合周任务（待方案确认，2026-09-04）
- [ ] 核验领星MCP是否能稳定提供父ASIN周粒度报表、自然周时间参数、店铺/站点维度和产品总览所需字段，先只读预览，不写入数据。
- [ ] 设计并确认唯一的周度触发器：父ASIN周报MCP读取替代现有由ASIN日快照聚合的周汇总任务；禁止双触发器、双来源累计或覆盖不同内容的历史周事实。
- [ ] 经用户确认后，实现周报受治理预览、完整性校验、幂等写入与产品总览周度来源切换；ASIN日数据仅保留单ASIN详情和库存规划参考。
- [ ] 验证父ASIN周报与用户此前上传周表的一致性、Run/Trace、异常复核、回滚与页面数据来源说明。
