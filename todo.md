...
- [ ] 新增Step2未确认大纲草稿保存接口，并在子图备注/主题编辑后自动持久化（代码、定向测试与前后端静态检查已通过，待真实空气套件验收）
- [ ] 验证刷新后场景备注、四个子图标题和锁定子图均正确水合
- [ ] 用空气套件自主填写场景备注并验证Step4/Step5下游目标同步
- [ ] 用空气套件真实填写Step2场景备注与子图主题，验证450ms自动保存后数据库step2UserEdit已更新
...
- [ ] 补充Step5场景子图模块级建议字段非空的回归测试或查询断言
