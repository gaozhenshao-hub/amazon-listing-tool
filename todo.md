...
- [ ] 核验青岛OSS原始响应Artifact归档的`UserDisable`拒绝原因及最小权限恢复方案；在恢复前保持批次哈希、Tool Run/Trace与行级草稿审计，不降低原始响应归档失败提示或自动应用边界。
...
- [ ] 在阿里云支持答复或账号级限制解除后，执行一次随机测试对象写入—元数据回读—删除的受控OSS探测；仅在成功后恢复原始响应Artifact归档，并核验业务同步审计边界未降低。
