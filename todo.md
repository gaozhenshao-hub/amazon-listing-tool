...
- [ ] 将领星FBA库存快照与广告关键词报表历史事实接入每日北京时间17:00计划，改为“完整性与异常校验通过即自动应用”，不再逐批等待人工确认
- [ ] 为库存和关键词建立与ASIN日数据相当的覆盖、分页、身份、字段与异常波动校验，并在失败时保留异常草稿和Trace
- [ ] 保持库存货期、缓冲、MOQ、成本与广告预算、竞价、否词、状态、结构等运营配置为人工确认，禁止自动改写
- [ ] 在同步页面展示自动应用策略、校验结果、异常阻断原因和Run/Trace审计，并补充定向回归与真实验收
- [ ] 为关键词历史事实增加来源Profile和稳定身份哈希，并以受控迁移及应用前预检防止计划重试重复写入
- [ ] 修复自动库存快照来源标识的字段长度兼容性，确保来源隔离不引入数据库写入截断
- [ ] 更新自动计划扩展后的运行时与页面契约断言，并新增库存与关键词自动应用的异常阻断回归
