...
- [ ] 修复活跃商品过滤后回补完成判定：以`applied`批次的店铺/日期窗口完整摘要识别已处理日期，而非以活跃日快照覆盖店铺数判断；对重复确认但未应用的批次保留失败审计且不得再次写入
- [ ] 将完整已应用的多日MCP批次纳入回补断点：覆盖摘要满足范围天数、店铺和店铺日期窗口完整时，范围内每个日期均视为已完成，避免重读2026-08-10至16和2026-08-17至23
- [ ] 将已创建且未应用的异常草稿日期纳入受保护断点：回补脚本跳过已审计的分页截断/超时窗口并继续其他日期，异常保持人工草稿且不重复读取或写入
