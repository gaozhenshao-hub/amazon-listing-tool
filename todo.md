...
- [ ] 使用7840控制板属性表完成真实导入验收，确认品牌、产品名称、ASIN、类目、规格、场景和卖点可写入Listing前置数据
- [ ] 定位7840控制板真实重试仍返回旧“缺少skillSlug”错误的原因，区分生产服务未切换、路由参数未透传或皇帝Skill运行失败
...
- [ ] 修复Rufus属性解析未保留源文件ASIN的问题，调整皇帝Skill输出契约、后端归一化和前台分析结果展示，确保源文件中的ASIN可进入Listing前置数据
