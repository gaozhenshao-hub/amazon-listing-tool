...
- [ ] 设计并创建 Step4 单图确认版本表，存储项目、图片序号、版本内容、状态和时间戳
- [ ] 单图确认与解锁改为写入独立版本记录，不再依赖会话 JSON 快照
- [ ] 页面 Step4 优先读取每张图的当前独立确认版本，整体发布仅汇总该版本
- [ ] 回填空气套件现有 Step4 图片版本并验证端到端一致性
- [ ] 为独立版本记录、页面水合和整体发布补充真实数据库回归测试
