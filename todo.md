...
- [ ] Phase 3：完成领星同步页面真实会话体验验收，展示数据域、店铺范围、批次摘要、Artifact归档、差异状态、确认后影响与日数据历史批次，并支持安全的只读审阅（页面增强已实现；待授权真实会话验收）
- [ ] 修复领星同步页面当前登录会话显示0个历史批次的问题：核对批次#3/#660001的工作空间与用户归属、`lingxingSync.list`过滤条件和会话工作空间解析，确保已审计批次可在授权工作空间只读展示
- [ ] Phase 4：完成受治理定时草稿同步基础能力验收，采用Heartbeat回调、幂等运行、QPS=1、失败摘要和管理员暂停/恢复；定时任务默认只生成草稿，不自动确认或写入业务事实（0175计划表、计划管理接口、每日/每周受保护回调与页面面板已实现；待真实启停验证）
- [ ] 每日北京时间17:00读取前一天领星ASIN日数据并生成待审核草稿；按店铺×日期幂等、QPS=1、失败可恢复，禁止自动确认/应用（代码已实现，待发布后显式启用与首轮运行验证）
- [ ] 每周自动生成父ASIN自然周汇总草稿与异常摘要，仅计算已确认日快照并保留数据缺失/分页截断/异常波动说明，禁止改写历史周度或人工财务数据（代码已实现，待发布后显式启用与首轮运行验证）
- [ ] 发布后由管理员在“自动草稿计划”面板显式启用每日17:00和每周一17:10计划；启用后首轮仅检查待审核批次、任务UID和失败摘要，不确认或应用业务数据
- [ ] 为`listSchedules`、`setScheduleEnabled`和`lingxingScheduledDraftHandler`补充行为级回归，覆盖Heartbeat创建、暂停/恢复、幂等跳过及失败摘要回写

- [ ] Phase 5：扩展Listing主数据、广告搜索词/投放目标和父ASIN原始流量的数据域预览，先完成官方MCP字段/粒度对账、草稿映射、Artifact与人工确认边界，不自动写入现有业务表
- [ ] 建立按数据域独立的同步规则目录：每个域必须配置粒度、店铺/站点范围、读取窗口、重复身份键、差异字段、缺失值、目标表、确认动作、写入策略、下游影响和定时计划，禁止复用不兼容规则
