...
- [ ] 完成领星同步服务的真实差异预览：基于现有产品/库存表填充`fieldDiffs`、`matchInfo`、`targetReference`并稳定分类新增/更新/未变化/跳过/需人工核对
- [ ] 完成模块三“从领星读取”同步预览入口的差异筛选与展示，明确呈现新增、更新、跳过和需人工核对，而非仅显示归一化草稿字段
...
- [ ] 在已确认具备权限的前提下，诊断产品表现和FBA库存零行响应的官方业务语义、店铺数据域与日期粒度；仅执行只读调用并保留脱敏审计
