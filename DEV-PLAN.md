...
**N3本品属性表模型可用性**：只读诊断确认失败记录源于受管外部模型出口不可用，而非文件解析、Skill注册、模型记录或凭证引用缺失。`analysis.rufus.attribute`的失败Run已被正确归类为`PROVIDER_UNAVAILABLE`；应用配置指向127.0.0.1:1088，但预配置的受限SSH出口隧道到远端连接超时并以255退出，端口未形成监听。已执行一次安全的隧道重启，仍未恢复监听；未重新解析任何用户上传文件或调用模型。随后确认青岛生产运行时同时具备外部与Forge受控凭据。已本地实现受治理的无迁移应急回退：外部模型保持首选，`manus-default`成为第一回退候选，并且仅该已登记的`manus_builtin`候选可由Skill Runner在外部路由失败后显式改用Forge；任意业务路由、客户端和上传文件均不能指定该覆盖。由于候选尝试上限为两次，这一顺序避免第二次仍命中共享失联隧道的外部模型。定向回归、ESLint、生产构建和Bundle预算已通过，尚待用户批准青岛无迁移原子发布；发布后的无内容健康验证不重试文件、不提交AI Job，任何真实用户文件重跑仍须另行确认。
