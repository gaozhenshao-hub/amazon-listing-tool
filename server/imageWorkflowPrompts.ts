...
**重要约束：双模式推荐**
- 【知识库约束模式】如果用户消息中提供了"知识库现有设计风格"列表，你推荐的每个风格方案的 name 字段必须严格从该列表中选取，不得凭空创造新的风格名称；此时每个方案的 source 字段设为 "kb"
- 【AI创意模式】如果知识库风格列表为空，则可以自由命名，但风格名称应简洁、专业（如"极简白底"、"深色科技"、"自然场景"等）；此时每个方案的 source 字段设为 "ai_creative"
- description、colorPalette 等其他字段可根据产品特性自由发挥
- 同一次推荐中所有方案的 source 字段必须一致（要么全是 "kb"，要么全是 "ai_creative"）
  ...
  "styleOptions": [
    {
      ...
      "source": "kb 或 ai_creative，表示该风格来源"
    }
  ],
  "recommendation": "综合推荐说明，建议选择哪1-2个风格及原因"
  "mode": "kb 或 ai_creative，本次推荐使用的模式"
}`;
...
图片大纲中的辅图固定为6张，编号2、3、4、5、6、7；imageReferences必须覆盖主图、全部辅图2-7和每个A+模块，**绝对不得遗漏辅图7**。
⚠️ 重要：辅图7是必须输出的，即使辅图7的内容与其他辅图相似，也必须单独输出一条imageNumber=7的记录。
