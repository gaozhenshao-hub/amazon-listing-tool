...
6. **进攻广告组 (Brand Offensive)**：竞对品牌词根下的关键词，主攻流量相对较大的美国本土卖家品牌，新品期可直接使用，成长期重点进攻
7. **自动广告组 (Auto Campaign)**：自动广告的优化建议
...
6. 进攻广告组：从关键词数据中识别分类为 brand_competitor 词根或 brand_offensive 策略的关键词，主要针对竞争对手品牌词根下的流量词。优先选择流量相对较大的美国本土卖家品牌词，新品期可直接使用，成长期重点进攻。adGroupType为"brand_offensive"
7. 否定词策略要防止广告组之间的内部竞争
8. 预算分配要根据产品所处阶段给出不同建议
9. 竞价建议要合理，参考PPC bid数据（如果有）
10. 所有金额使用美元
11. orderVolumeProjection必须基于关键词搜索量数据合理预估，给出保守和乐观两个范围
12. organicRankingEstimate中的topKeywords选取搜索量最高的3-5个核心词，预估自然排名提升路径
13. 每个阶段的自然出单占比应随时间递增（新品期10-20%，成长期30-50%，成熟期50-70%）
14. 首页首位出单量预估要结合SPR数据和关键词搜索量
15. **【最重要】所有keyword字段、negativeKeywords字段必须保留英文原文，严禁翻译成中文。从输入数据中选取的关键词必须原样使用，不得翻译、改写或转换语言。**`;
