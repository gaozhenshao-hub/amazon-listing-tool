    ...
    .mutation(async ({ ctx, input }) => {
      ...
      // [Emperor] 优先调用 Emperor Skill: listing.competitor.analyze
          ...
          try {
            ...
            const _emperorRes = await runSkillViaEmperor("listing.competitor.analyze", { context: JSON.stringify(input).slice(0, 3000) });
          ...
          } catch (_e) { console.warn("[Emperor] kbListings.ts fallback:", _e); }
