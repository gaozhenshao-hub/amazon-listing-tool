  ...
  const parseSalesData = async (rows: any[]) => {
    const products = rows.map((r: any) => {
      ...
      return {
        ...
        bsrGrowthRate: pick("大类BSR增长率", "BSR Growth Rate", "BSR增长率"),
        fbaFee: pick("FBA费用", "FBA费用($)", "FBA Fee"),
        grossMargin: pick("毛利率", "Gross Margin", "Profit Margin"),
        monthlySalesGrowth: pick("月销量增长率", "Monthly Sales Growth"),
        childSales: pickNum("子体销量", "Child Sales"),
        childRevenue: pickNum("子体销售额", "子体销售额($)", "Child Revenue"),
        monthlyNewReviews: pickNum("月新增评分数", "Monthly New Reviews"),
        reviewRate: pick("留评率", "Review Rate"),
        ...
        hasAPlus: pickNum("A+页面", "A+ Content") ? 1 : (pick("A+页面", "A+ Content") === "是" ? 1 : 0),
        hasVideo: pickNum("视频介绍", "Video") ? 1 : (pick("视频介绍", "Video") === "是" ? 1 : 0),
        hasBrandStory: pickNum("品牌故事", "Brand Story") ? 1 : (pick("品牌故事", "Brand Story") === "是" ? 1 : 0),
        hasAmazonChoice: pickNum("Amazon's Choice", "AC标记") ? 1 : (pick("Amazon's Choice", "AC标记") === "是" ? 1 : 0),
