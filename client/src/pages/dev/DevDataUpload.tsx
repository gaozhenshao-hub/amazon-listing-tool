  ...
  const parseSalesData = async (rows: any[]) => {
    const products = rows.map((r: any) => {
      ...
      return {
        ...
        category: pick("大类目", "类目", "Category", "Main Category"),
        ...
        bulletPoints: pick("产品卖点", "五点描述", "Bullet Points"),
        specifications: pick("详细参数", "规格参数", "Specifications"),
        // --- 全景分析表扩展字段 ---
        parentAsin: pick("父ASIN", "Parent ASIN", "parentAsin"),
        sku: pick("SKU", "sku"),
        productLink: pick("商品链接", "产品链接", "链接", "Product Link", "URL"),
        categoryPath: pick("类目路径", "Category Path", "Full Category"),
        bsrLarge: pickNum("大类BSR", "Large Category BSR"),
        bsrSmall: pickNum("小类BSR", "Small Category BSR"),
        bsrGrowthRate: String(pick("大类BSR增长率", "BSR Growth Rate", "BSR增长率") ?? ""),
        fbaFee: String(pick("FBA($)", "FBA费用", "FBA费用($)", "FBA Fee") ?? ""),
        grossMargin: String(pick("毛利率", "Gross Margin", "Profit Margin") ?? ""),
        monthlySalesGrowth: String(pick("月销量增长率", "Monthly Sales Growth") ?? ""),
        childSales: pickNum("子体销量", "Child Sales"),
        childRevenue: pickNum("子体销售额", "子体销售额($)", "Child Revenue"),
        monthlyNewReviews: pickNum("月新增\n评分数", "月新增评分数", "Monthly New Reviews"),
        reviewRate: String(pick("留评率", "Review Rate") ?? ""),
        lqs: pickNum("LQS", "Listing质量分", "Listing Quality Score"),
        sellerCount: pickNum("卖家数", "Seller Count"),
        listingDays: pickNum("上架天数", "Listing Days", "Days Since Listed"),
        buyboxSeller: pick("Buybox卖家", "BuyBox Seller"),
        buyboxType: pick("BuyBox类型", "BuyBox Type"),
        hasAPlus: (() => { const v = pick("A+页面", "A+ Content"); return (v === "Y" || v === "是" || v === "1" || v === 1) ? 1 : 0; })(),
        hasVideo: (() => { const v = pick("视频介绍", "Video"); return (v === "Y" || v === "是" || v === "1" || v === 1) ? 1 : 0; })(),
        hasBrandStory: (() => { const v = pick("品牌故事", "Brand Story"); return (v === "Y" || v === "是" || v === "1" || v === 1) ? 1 : 0; })(),
        hasAmazonChoice: (() => { const v = pick("Amazon's Choice", "AC标记"); return (v === "Y" || v === "是" || v === "1" || v === 1) ? 1 : 0; })(),
        productWeight: pick("商品重量", "Product Weight", "Item Weight"),
        productSize: pick("商品尺寸", "Product Size", "Item Dimensions"),
        packageWeight: pick("包装重量", "Package Weight", "Shipping Weight"),
        packageSize: pick("包装尺寸", "Package Size", "Package Dimensions"),
