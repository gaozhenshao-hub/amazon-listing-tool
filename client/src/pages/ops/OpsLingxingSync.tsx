...
export default function OpsLingxingSync() {
  ...
  const activeBatchReviewBlocked = ["product_performance_daily", "fba_inventory", "ad_keyword", "parent_asin_weekly_mcp"].includes(activeBatchDomain) && (
  ...
  return <div className="space-y-6">
    ...
    <Card>
      ...
      <Card className="border-amber-200 bg-amber-50/40"><CardContent className="flex gap-3 pt-6 text-sm text-amber-900"><FileCheck2 className="mt-0.5 h-5 w-5 shrink-0" /><p><b>人工确认与自动应用边界：</b>领星手动读取先生成独立草稿批次。受治理的每日产品日数据、FBA库存快照、广告关键词历史和父ASIN周汇总计划，会在分页、授权范围、身份、字段和异常校验通过后直接追加对应历史事实；冲突或异常会阻断并保留审计。库存货期、缓冲、MOQ、成本及广告预算、竞价、否词、状态与结构均不会由自动计划修改。</p></CardContent></Card>
      ...
      <Card className="border-violet-200 bg-violet-50/40"><CardHeader><CardTitle className="flex items-center gap-2 text-base"><CalendarClock className="h-4 w-4" />受治理自动计划</CardTitle><CardDescription>每日产品、FBA库存、广告关键词与父ASIN周报仅在完整性校验通过后自动追加；冲突或异常会阻断。库存与广告运营配置不会自动写入或修改。</CardDescription></CardHeader><CardContent className="grid gap-3 lg:grid-cols-2">{scheduledDrafts.map((plan) => {
