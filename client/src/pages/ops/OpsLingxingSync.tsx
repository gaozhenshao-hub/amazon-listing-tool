...
const domains = [
  ...
  { value: "parent_asin_weekly_mcp", label: "父ASIN周报（产品总览）", detail: "按上一完整自然周读取美国站全店父ASIN周报；仅在店铺覆盖、分页、周范围与身份校验完整时直接幂等追加周度事实" },
  ...
  { value: "ad_campaign_mcp", label: "广告活动日事实（产品详情）", detail: "每日18:00读取前一天美国站已授权Profile的广告活动状态与预算；完整批次自动写入独立活动事实，不修改预算、竞价或状态" },
  { value: "ad_product_mcp", label: "广告商品日事实（产品详情）", detail: "每日18:10读取前一天美国站已授权Profile的广告商品事实；通过子ASIN精确关联父ASIN后自动写入，供产品详情按自然周展示" },
...
const phase5DetailColumns: Record<string, Array<{ key: string; label: string }>> = {
  ...
  ad_campaign_mcp: [{ key: "profileId", label: "Profile" }, { key: "campaignId", label: "活动ID" }, { key: "campaignName", label: "活动" }, { key: "campaignStatus", label: "状态" }, { key: "budget", label: "预算" }, { key: "reportDate", label: "报告日" }],
  ad_product_mcp: [{ key: "profileId", label: "Profile" }, { key: "advertisedAsin", label: "广告ASIN" }, { key: "parentAsin", label: "父ASIN" }, { key: "campaignName", label: "活动" }, { key: "adSpend", label: "花费" }, { key: "reportDate", label: "报告日" }],
};

type DraftEdit = Record<number, {
  selected?: boolean;
  normalizedData?: Record<string, unknown>;
  rowStatus?: "new" | "changed" | "unchanged" | "unmatched" | "needs_review" | "skipped";
}>;
...
export default function OpsLingxingSync() {
  ...
  const isAdDomain = ["ad_campaign", "ad_keyword", "ad_campaign_mcp", "ad_product_mcp", "ad_search_term", "ad_targeting"].includes(domain);
  ...
  const applyAdsMutation = trpc.lingxingSync.applyConfirmedAds.useMutation({
    onSuccess: (result) => {
      toast.success("已写入广告事实", { description: `写入 ${result.importedRows} 行，修订 ${result.revisedRows || 0} 行，跳过 ${result.skippedRows || 0} 行。系统未修改预算、竞价、投放状态或广告组合。` });
      void batchQuery.refetch();
      void historyQuery.refetch();
    },
  ...
  const activeBatchReviewBlocked = ["product_performance_daily", "fba_inventory", "ad_keyword", "ad_campaign_mcp", "ad_product_mcp", "parent_asin_weekly_mcp"].includes(activeBatchDomain) && (
    ...
    || (Number(activeSummary.profilesExpected || 0) > 0 && Number(activeSummary.profilesExpected) !== Number(activeSummary.profilesRead || 0))
    || (Number(activeSummary.profileDateWindowsExpected || 0) > 0 && Number(activeSummary.profileDateWindowsExpected) !== Number(activeSummary.profileDateWindowsRead || 0))
  );
  const scheduledDrafts = [
    ...
    { dataDomain: "ad_campaign_mcp" as const, title: "每日广告活动事实", timing: "每天北京时间 18:00 · 读取前一天", detail: "美国站全部已授权广告Profile逐页读取；仅在全Profile覆盖、分页完整与字段有效时自动写入独立活动事实", autoApply: true },
    { dataDomain: "ad_product_mcp" as const, title: "每日广告商品事实", timing: "每天北京时间 18:10 · 读取前一天", detail: "美国站全部已授权广告Profile逐页读取；仅在全Profile覆盖、分页完整、广告实体唯一且子ASIN精确映射父ASIN时自动写入独立商品事实", autoApply: true },
  ...
  const runPreview = () => {
    ...
    if ((domain === "product_performance" || domain === "product_performance_daily" || domain === "parent_asin_weekly_mcp" || domain === "order_profit" || isAdDomain) && (!startDate || !endDate)) return toast.error("请选择开始和结束日期");
  ...
  const retryReviewDate = (entry: any) => {
    ...
    const retryIsAd = ["ad_keyword", "ad_campaign_mcp", "ad_product_mcp"].includes(retryDomain);
    const retryStoreId = String(scope.storeId || (retryIsAd ? "ALL_US_AD_PROFILES" : "ALL_US"));
    const retryProfileId = retryIsAd ? String(scope.profileId || "ALL_US_AD_PROFILES") : "";
  ...
  return <div className="space-y-6">
    ...
    <Card>
      ...
      <CardContent className="grid gap-4 md:grid-cols-2 xl:grid-cols-5">
        ...
        <div className="space-y-2"><Label>广告 Profile ID（广告报表必填）</Label><select className="h-10 w-full rounded-md border bg-background px-3 text-sm" value={profileId} onChange={(event) => selectAdProfile(event.target.value)} disabled={!isAdDomain || adProfilesQuery.isLoading}><option value="">{isAdDomain ? "请选择官方广告授权店铺" : "广告数据域时选择"}</option>{["ad_keyword", "ad_campaign_mcp", "ad_product_mcp", "ad_search_term", "ad_targeting"].includes(domain) && usAdProfileIds ? <option value="ALL_US_AD_PROFILES">美国站全部广告授权Profile（{usAdProfileIds.split(",").length}家）</option> : null}{(adProfilesQuery.data || []).map((profile) => <option key={profile.profileId} value={profile.profileId}>{profile.name}{profile.country ? ` · ${profile.country}` : ""} · {profile.profileId}</option>)}</select>{isAdDomain && !adProfilesQuery.isLoading && !(adProfilesQuery.data || []).length ? <p className="text-xs text-amber-700">未读取到广告授权Profile；请检查领星广告授权范围。</p> : null}</div>
      ...
      <Card className="border-amber-200 bg-amber-50/40"><CardContent className="flex gap-3 pt-6 text-sm text-amber-900"><FileCheck2 className="mt-0.5 h-5 w-5 shrink-0" /><p><b>人工确认与自动应用边界：</b>领星手动读取先生成独立草稿批次。受治理的每日产品日数据、FBA库存快照、广告关键词、广告活动、广告商品及父ASIN周汇总计划，会在分页、授权范围、身份、字段和异常校验通过后直接写入各自独立事实；冲突或异常会阻断并保留审计。广告商品仅在子ASIN可精确关联父ASIN时写入；库存货期、缓冲、MOQ、成本及广告预算、竞价、否词、状态与结构均不会由自动计划修改。</p></CardContent></Card>
      ...
      <Card className="border-violet-200 bg-violet-50/40"><CardHeader><CardTitle className="flex items-center gap-2 text-base"><CalendarClock className="h-4 w-4" />受治理自动计划</CardTitle><CardDescription>每日产品、FBA库存、广告关键词、广告活动、广告商品与父ASIN周报仅在完整性校验通过后自动追加；冲突或异常会阻断。库存与广告运营配置不会自动写入或修改。</CardDescription></CardHeader><CardContent className="grid gap-3 lg:grid-cols-2">{scheduledDrafts.map((plan) => {
