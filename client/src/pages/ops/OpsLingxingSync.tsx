...
export default function OpsLingxingSync() {
  ...
  const schedulesQuery = trpc.lingxingSync.listSchedules.useQuery(undefined, { retry: false });
  ...
  const scheduleMutation = trpc.lingxingSync.setScheduleEnabled.useMutation({
    onSuccess: (result) => {
      toast.success(result.enabled ? "已启用自动草稿计划" : "已暂停自动草稿计划", { description: "计划只生成待审核批次，绝不会自动确认或写入业务数据。" });
  ...
  const scheduledDrafts = [
    { dataDomain: "product_performance_daily" as const, title: "每日产品日数据草稿", timing: "每天北京时间 17:00 · 读取前一天", detail: "美国站全店逐日读取，仅生成待审核ASIN日快照草稿" },
    { dataDomain: "parent_asin_weekly_rollup" as const, title: "每周父ASIN汇总草稿", timing: "每周一北京时间 17:10 · 汇总上一自然周", detail: "仅汇总已确认日快照，生成父ASIN周度摘要与异常提示" },
  ];
  ...
  return <div className="space-y-6">
    ...
    <Card>
      ...
      <Card className="border-slate-200 bg-slate-50/50"><CardHeader className="pb-2"><CardTitle className="text-base">Phase 5 · 只读预览准备域</CardTitle><CardDescription>以下映射已按官方MCP参数完成设计，但尚未开放草稿写入或自动计划；避免未验证字段进入现有业务表。</CardDescription></CardHeader><CardContent className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">{LINGXING_SYNC_RULES.filter((rule) => ["listing_master", "ad_search_term", "ad_targeting", "parent_asin_traffic"].includes(rule.domain)).map((rule) => <div key={rule.domain} className="rounded-md border bg-background p-3 text-sm"><div className="flex items-start justify-between gap-2"><p className="font-medium">{rule.label}</p><Badge variant="outline">只读准备</Badge></div><p className="mt-2 text-xs text-muted-foreground">来源：{rule.source}</p><p className="mt-1 text-xs text-muted-foreground">身份键：{rule.identity}</p><p className="mt-2 text-xs">{rule.confirmation}</p></div>)}</CardContent></Card>

      <Card className="border-violet-200 bg-violet-50/40"><CardHeader><CardTitle className="flex items-center gap-2 text-base"><CalendarClock className="h-4 w-4" />自动草稿计划</CardTitle><CardDescription>按数据域独立运行。任务只读取领星并创建待审核批次；不会自动确认、应用、覆盖历史数据或修改广告设置。</CardDescription></CardHeader><CardContent className="grid gap-3 lg:grid-cols-2">{scheduledDrafts.map((plan) => {
        const schedule = (schedulesQuery.data || []).find((item: any) => item.dataDomain === plan.dataDomain);
        const enabled = Boolean(schedule?.enabled);
        return <div key={plan.dataDomain} className="rounded-lg border bg-background p-4"><div className="flex items-start justify-between gap-3"><div><p className="font-medium">{plan.title}</p><p className="mt-1 text-xs text-muted-foreground">{plan.timing}</p></div><Badge variant={enabled ? "default" : "outline"}>{enabled ? "运行中" : "未启用"}</Badge></div><p className="mt-3 text-sm text-muted-foreground">{plan.detail}</p><div className="mt-3 grid gap-1 text-xs text-muted-foreground"><span>最近状态：{schedule?.lastStatus || "尚未运行"}</span><span>最近草稿：{schedule?.lastBatchId ? `#${schedule.lastBatchId}` : "—"}</span>{schedule?.lastError ? <span className="text-amber-700">最近错误：{schedule.lastError}</span> : null}</div><Button className="mt-4 w-full" variant={enabled ? "outline" : "default"} onClick={() => scheduleMutation.mutate({ dataDomain: plan.dataDomain, enabled: !enabled })} disabled={scheduleMutation.isPending}>{scheduleMutation.isPending && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}{enabled ? "暂停计划" : "启用计划"}</Button></div>;
      })}</CardContent></Card>
