...
export default function OpsLingxingSync() {
  ...
  const scheduledDrafts = [
    ...
    { dataDomain: "parent_asin_weekly_rollup" as const, title: "每周父ASIN汇总", timing: "每周一北京时间 17:10 · 汇总上一自然周", detail: "仅汇总已确认日快照；完整性校验通过后直接幂等追加父ASIN周事实，冲突或覆盖异常会阻断并保留审计", autoApply: true },
  ];
  ...
  return <div className="space-y-6">
    ...
    <Card>
      ...
      <Card className="border-violet-200 bg-violet-50/40"><CardHeader><CardTitle className="flex items-center gap-2 text-base"><CalendarClock className="h-4 w-4" />受治理自动计划</CardTitle><CardDescription>每日产品、FBA库存、广告关键词与父ASIN周汇总仅在完整性校验通过后自动追加；冲突或异常会阻断。库存与广告运营配置不会自动写入或修改。</CardDescription></CardHeader><CardContent className="grid gap-3 lg:grid-cols-2">{scheduledDrafts.map((plan) => {
