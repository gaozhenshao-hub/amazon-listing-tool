...
import {
  ...
  AlertTriangle, ClipboardList, SkipForward, Trash2, History, Clock,
} from "lucide-react";
import {
...
export default function OpsPlanImportTab() {
  ...
  // ─── Queries ───
  const importHistory = trpc.productOps.listImportHistory.useQuery({ importType: "plan" });
  ...
  const deleteImportHistoryMut = trpc.productOps.deleteImportHistory.useMutation({
    onSuccess: (data) => {
      ...
      toast.success("导入记录已删除", {
        description: `已级联删除 ${data.deletedRecords} 条运营计划数据`,
      });
    },
    onError: (err) => {
      ...
      {(importHistory.data && importHistory.data.length > 0) && (
        <Card>
          <CardHeader className="pb-3">
            ...
            <CardDescription>删除导入记录将同步清除该次导入创建的所有运营计划数据</CardDescription>
          </CardHeader>
          <CardContent>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              此操作将永久删除该次导入创建的所有运营计划数据（包括计划、行动项和总结），且不可恢复。确定要继续吗？
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
